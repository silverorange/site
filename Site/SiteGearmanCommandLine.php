...
/**
 * Optional helper class for building command-line interfaces for Gearman
 * workers
 *
 * This adds all the default options and arguments required by Gearman
 ...
 * <code>
 * <?php
 * $parser = SiteGearmanCommandLine::fromXMLFile($my_file);
 * ?>
 * </code>
 *
 * @package   Site
 ...
 */
abstract class SiteGearmanCommandLine
{
		...
		if (!isset($parser->options['port'])) {
			$parser->addOption(
				...
				array(
					...
					'default'     => 4730,
					'description' => Site::_(
						'Port of gearmand server. If not specified, port '.
						'4730 is used.'
					)
				)
			);
		...
		if (!isset($parser->arguments['address'])) {
			$parser->addArgument(
				...
				array(
					...
					'default'     => '127.0.0.1',
					'description' => Site::_(
						'Gearmand server address. If not specified, '.
						'127.0.0.1 is used.'
					)
				)
			);
