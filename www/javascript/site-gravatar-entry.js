function SiteGravatarEntry(id, size, default_image)
{
	this.id = id;
	this.timeout = null;
	this.size = size;
 ...
 * @var Number
 */
SiteGravatarEntry.TYPING_PERIOD = 500;

SiteGravatarEntry.prototype.init = function()
{
	this.input = document.getElementById(this.id);
	this.preview = document.getElementById(this.id + '_preview');
	this.link = document.getElementById(this.id + '_link');
	this.loading_image = YAHOO.util.Dom.getStyle(
		this.preview,
		'backgroundImage'
	);
...
SiteGravatarEntry.prototype.getNormalizedValue = function(e)
{
	return this.input.value.replace(/^\s+|\s+$/g, '');
};

SiteGravatarEntry.prototype.handleKeypress = function(e)
{
	if (this.timeout) {
...
SiteGravatarEntry.prototype.updatePreview = function(email)
{
	var src;
	var md5_email = SiteGravatarEntry.md5(email);
	var params = this.getParams();

	if (location.protocol === 'https:') {
		src = 'https://secure.gravatar.com/avatar/' + md5_email + '?' + params;
	} else {
		src = 'http://www.gravatar.com/avatar/' + md5_email + '?' + params;
	}
...
{
	var params = ['s=' + this.size];

	if (this.default_image) {
		params.push('d=' + encodeURIComponent(this.default_image));
	}
 ...
 *
 * Copyright 1999-2010 Joseph Myers, Paul Johnston, Greg Holt,
...
(function() {
	...
	SiteGravatarEntry.md5 = function(s)
	{
		return hex(md51(s));
	};
